...
impl App {
    ...
    pub fn get_help_content() -> Vec<String> {
        vec![
            ...
            "".to_string(),
            "SHELL INTEGRATION - BASH WRAPPER FUNCTION".to_string(),
            "  To enable directory navigation with 'dt' command, add this function".to_string(),
            "  to your shell configuration file:".to_string(),
            "".to_string(),
            "  For Bash (~/.bashrc):".to_string(),
            "    dt() {".to_string(),
            "      local result=$(command dtree \"$@\")".to_string(),
            "      # Only cd if result is a valid directory (ignores help, version, errors)".to_string(),
            "      if [ -n \"$result\" ] && [ -d \"$result\" ]; then".to_string(),
            "        cd \"$result\" || return".to_string(),
            "      fi".to_string(),
            "    }".to_string(),
            "".to_string(),
            "  For Zsh (~/.zshrc):".to_string(),
            "    dt() {".to_string(),
            "      local result=$(command dtree \"$@\")".to_string(),
            "      # Only cd if result is a valid directory (ignores help, version, errors)".to_string(),
            "      if [ -n \"$result\" ] && [ -d \"$result\" ]; then".to_string(),
            ...
            "    1. Copy the function to your ~/.bashrc or ~/.zshrc".to_string(),
