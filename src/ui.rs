...
pub fn get_help_content() -> Vec<String> {
    vec![
        ...
        "".to_string(),
        "BINARY FILE SUPPORT".to_string(),
        "  When you open a binary file (executables, images, archives, etc.):".to_string(),
        "    • dtree automatically detects binary files (checks for NULL bytes)".to_string(),
        "    • Shows informational message with file details instead of raw content".to_string(),
        "    • Displays file type, size, and permissions".to_string(),
        "    • Press 'e' to open in hex editor (configurable in config.toml)".to_string(),
        "".to_string(),
        "  Supported file types:".to_string(),
        "    • Executables / Libraries (.exe, .dll, .so, .dylib, .bin)".to_string(),
        "    • Archives (.zip, .tar, .gz, .7z, .rar)".to_string(),
        "    • Images (.png, .jpg, .gif, .bmp, .webp)".to_string(),
        "    • Video (.mp4, .mkv, .avi, .mov)".to_string(),
        "    • Audio (.mp3, .wav, .flac, .ogg)".to_string(),
        "    • Documents (.pdf, .doc, .xls, .ppt)".to_string(),
        "    • Databases (.db, .sqlite)".to_string(),
        "    • And more...".to_string(),
        "".to_string(),
        "  Hex editor configuration (config.toml):".to_string(),
        "    [behavior]".to_string(),
        "    hex_editor = \"hexyl\"  # Default: hexyl".to_string(),
        "".to_string(),
        "  Install hexyl: cargo install hexyl".to_string(),
        "  Or use any other hex editor: xxd, hd, hexdump, etc.".to_string(),
        "".to_string(),
        ...
        "    • In fullscreen mode, ONLY viewer keys work (see above)".to_string(),
        "    • All other keys are disabled (search, bookmarks, tree navigation, etc.)".to_string(),
        "    • When launched via CLI (dt -v file.txt), file mode is enabled automatically".to_string(),
        "    • This allows Ctrl+j/k to navigate between files in the same directory".to_string(),
        "".to_string(),
        ...
        "    dt -bm remove work   Remove bookmark 'work'".to_string(),
        "    dt -v file.txt       View file, navigate, cd on exit (with q)".to_string(),
        ...
        "    • dt -v <file> = View file, then cd to selected dir on exit (q)".to_string(),
